...
/**
 * ChainSync N2N server — serves headers to a connected peer.
 *
 * Similar to ChainSyncServer (N2C) but with N2N wire format:
 *   - MsgRollForward sends era-wrapped headers (not full blocks)
 *   - Uses protocol ID 2 (not 5)
 *   - Responses use sendResponse (responder direction)
 *
 * Maintains a cursor (current block height) and serves headers from RocksDB.
 * When the peer catches up to the tip, subscribes to the ChainEvent topic
 * and waits for new blocks.
 */
final class ChainSyncServerN2N(
    mux: MuxDemuxer,
    store: RocksDbStore,
    tipTopic: Topic[IO, ChainEvent]
):

  private val logger  = Slf4jLogger.getLoggerFromName[IO]("stretto.node.ChainSyncServerN2N")
  ...
  /**
   * Encode MsgRollForward for N2N: [2, eraWrappedHeader, tip].
   *
   * N2N sends the era-wrapped header directly (as stored in the headers CF).
   * The header is already in [era_tag, tag24(headerBytes)] format from ChainSync.
   */
  private def encodeMsgRollForward(eraWrappedHeader: ByteVector, tip: Tip): ByteVector =
    cborArrayHeader(3) ++ cborUInt(2) ++ eraWrappedHeader ++ encodeTip(tip)
  ...
  /** Run the ChainSync N2N server loop for a single peer. */
  def serve: IO[Unit] =
    for
  ...
  private def serverLoop(cursorRef: Ref[IO, BlockNo]): IO[Unit] =
    recvClientMsg.flatMap {
      ...
      case ClientMsg.Done =>
        logger.debug("ChainSync N2N: peer done")
    }
    ...
    for
      ...
      _ <- maxHeight match
        case Some(maxH) if nextHeight.blockNoValue <= maxH.blockNoValue =>
          serveHeaderAtHeight(cursorRef, nextHeight, tip)
        case _ =>
          sendMsg(encodeMsgAwaitReply) *>
            waitForNewBlock(cursorRef)
    yield ()

  /** Read header at the given height and send MsgRollForward. */
  private def serveHeaderAtHeight(
      cursorRef: Ref[IO, BlockNo],
      height: BlockNo,
      tip: Tip
  ): IO[Unit] =
    store.getPointByHeight(height).flatMap {
      case Some(point) =>
        store.getHeader(point).flatMap {
          case Some(headerData) =>
            cursorRef.set(height) *>
              sendMsg(encodeMsgRollForward(headerData, tip))
          case None =>
            // Header data missing — still advance cursor
            cursorRef.set(height) *>
  ...
  /** Wait for a new block from the topic, then serve the header. */
  ...
  // ---------------------------------------------------------------------------
  // Client message decoding (same wire format as N2C ChainSync)
  // ---------------------------------------------------------------------------
  ...
  private def recvClientMsg: IO[ClientMsg] =
    mux.recvProtocol(protoId).flatMap { payload =>
      decodeClientMsg(payload) match
        ...
        case Left(err) =>
          IO.raiseError(new RuntimeException(s"N2N ChainSync client decode error: $err"))
    }
    ...
    if bytes.isEmpty then Left("empty payload")
      ...
      for
        ...
        result <- tag match
          ...
          case _ => Left(s"unknown N2N ChainSync client tag: $tag")
      yield result
